    ...
    while True:
        ...
        if len(blue_points) >= 2 and point_rouge:
            ...
            with open("position_reelle.txt", "w") as f:
                f.write(f"{local_x_cm:.2f},{local_y_cm:.2f}")

            # Affichage ou hors cadre
            if origin_x <= cX <= max_x and min_y <= cY <= origin_y:
                print(f"Position du point rouge : X = {local_x_cm:.2f} cm, Y = {local_y_cm:.2f} cm")
            else:
